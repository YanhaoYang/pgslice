...
module PgSlice
  class Unswap
    ...
    def run
      ...
      abort "Table not found: #{retired_table}" unless conn.table_exists?(retired_table)
      abort "Table already exists: #{intermediate_table}" if conn.table_exists?(intermediate_table)

      queries = [
        "ALTER TABLE #{conn.quote_ident(table)} RENAME TO #{conn.quote_ident(intermediate_table)};",
        "ALTER TABLE #{conn.quote_ident(retired_table)} RENAME TO #{conn.quote_ident(table)};"
      ]
